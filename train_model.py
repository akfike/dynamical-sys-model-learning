...
# Update feature extraction to include both V_C and I_L as inputs
X_train, y_train = train_data[:, :3], train_data[:, 3]  # Assuming you're predicting next V_C
...
model_save_dir = 'saved_models/RLC'
...
for architecture in network_architectures:
    # Adjusted to accept 3 input features
    model = keras.models.Sequential()
    model.add(keras.layers.Dense(architecture[0], activation='relu', input_shape=(3,)))
    for units in architecture[1:]:
        model.add(keras.layers.Dense(units, activation='relu'))
    model.add(keras.layers.Dense(1))  # Assuming you're predicting the next V_C
...
X_new_test, y_new_test = new_test_data[:, :3], new_test_data[:, 3]  # Updated to correct slicing
...
for architecture, history in training_histories.items():
    ...
    model = keras.models.load_model(f'saved_models/RLC/model_architecture_{architecture}.keras')  # Assuming models were saved
